  ...
  nodeHandle_(nodeHandle)
{
  ...
  service_ = nodeHandle_.advertiseService("/stop_husky", &HuskyHighlevelController::stopCallback, this);
}
...
bool HuskyHighlevelController::stopCallback(std_srvs::SetBool::Request& request,
                                               std_srvs::SetBool::Response& response) {
  if (request.data == true) {
    stop = true;
    response.message = "Husky stopped.";

    ROS_INFO("Husky stopped.");
  } else {
    stop = false;
    response.message = "Husky started.";

    ROS_INFO("Husky started.");
  }
